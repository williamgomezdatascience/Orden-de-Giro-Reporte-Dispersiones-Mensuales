/* ============================================================================
   DETALLE_GIRO_ORACLE_2V2.sql
   Conversion de SQL Server -> Oracle, compatible con GoAnywhere MFT (JDBC).
   Version de origen: 2_v2.

   Diferencias respecto a la version 1_v2 (ver DETALLE_GIRO_ORACLE.sql):
   - Las tablas derivadas PM1..PM10 (InformacionPM) ahora se leen desde el
     servidor [180.26.149.71].[OriginacionPROD] (productivo, NO snapshot) en
     vez de [180.26.149.41].[OriginacionPROD_SNAPSHOT]. El resto de los joins
     (ori, ope, df, est, seg) se mantienen sobre OriginacionPROD_SNAPSHOT,
     igual que en 1_v2. Se asume un DB LINK distinto para este origen:
     originacionprod.tabla@dbl_originacionprod (pendiente de confirmar nombre
     real con el DBA).
   - CANAL GIRO agrega una rama nueva al inicio: cuando
     Seg.bMantenimientoPrepagado = 1 y dre.IdDesembolsoDetalle = 6, devuelve
     'GIRO ACH' directamente (antes de evaluar las ramas de PAGO ALTAIR/BOT).
   - El filtro IdDesembolsodetalle pasa de IN (1,2,6,7) a IN (1,6,7) (se quita
     el valor 2).
   - La comparacion final contra CORTE DISPERSION pasa de '=' a '>'.
   - Se agrega un comentario inerte "--and a.Solicitud <> 1219460" en el
     original (no es un filtro activo).
   - OBSERVACIONES tiene una linea comentada adicional (bBlindaje) sin
     efecto, no se traduce a SQL ejecutable.
   El resto del query (10 columnas, NUMEROCUENTA, VLRGIRAR, etc.) es igual a
   la version 1_v2.
...
FROM (
    SELECT DISTINCT
        ...
        '$' || REPLACE(bdpremierprueba.formatmoney(
                  ...
                  END, 30,'','',',',''),' ','') AS VLRGIRAR,
        CASE
            WHEN seg.bMantenimientoPrepagado = 1 AND dre.IdDesembolsoDetalle = 6 THEN 'GIRO ACH'
            -- (en el original hay una rama comentada: WHEN Seg.bBlindaje = 1 AND idDesembolsoDetalle = 7 THEN 'GIRO ACH'; sin efecto)
    ...
    LEFT JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY DatosFactura ORDER BY idinformacionPM ASC) AS conteo, t.*
                 FROM originacionprod.informacionpm@dbl_originacionprod t) PM1
           ON PM1.DatosFactura = df.idDatosFactura AND PM1.conteo = 1
    LEFT JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY DatosFactura ORDER BY idinformacionPM ASC) AS conteo, t.*
                 FROM originacionprod.informacionpm@dbl_originacionprod t) PM2
           ON PM2.DatosFactura = df.idDatosFactura AND PM2.conteo = 2
    LEFT JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY DatosFactura ORDER BY idinformacionPM ASC) AS conteo, t.*
                 FROM originacionprod.informacionpm@dbl_originacionprod t) PM3
           ON PM3.DatosFactura = df.idDatosFactura AND PM3.conteo = 3
    LEFT JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY DatosFactura ORDER BY idinformacionPM ASC) AS conteo, t.*
                 FROM originacionprod.informacionpm@dbl_originacionprod t) PM4
           ON PM4.DatosFactura = df.idDatosFactura AND PM4.conteo = 4
    LEFT JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY DatosFactura ORDER BY idinformacionPM ASC) AS conteo, t.*
                 FROM originacionprod.informacionpm@dbl_originacionprod t) PM5
           ON PM5.DatosFactura = df.idDatosFactura AND PM5.conteo = 5
    LEFT JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY DatosFactura ORDER BY idinformacionPM ASC) AS conteo, t.*
                 FROM originacionprod.informacionpm@dbl_originacionprod t) PM6
           ON PM6.DatosFactura = df.idDatosFactura AND PM6.conteo = 6
    LEFT JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY DatosFactura ORDER BY idinformacionPM ASC) AS conteo, t.*
                 FROM originacionprod.informacionpm@dbl_originacionprod t) PM7
           ON PM7.DatosFactura = df.idDatosFactura AND PM7.conteo = 7
    LEFT JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY DatosFactura ORDER BY idinformacionPM ASC) AS conteo, t.*
                 FROM originacionprod.informacionpm@dbl_originacionprod t) PM8
           ON PM8.DatosFactura = df.idDatosFactura AND PM8.conteo = 8
    LEFT JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY DatosFactura ORDER BY idinformacionPM ASC) AS conteo, t.*
                 FROM originacionprod.informacionpm@dbl_originacionprod t) PM9
           ON PM9.DatosFactura = df.idDatosFactura AND PM9.conteo = 9
    LEFT JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY DatosFactura ORDER BY idinformacionPM ASC) AS conteo, t.*
                 FROM originacionprod.informacionpm@dbl_originacionprod t) PM10
    ...
    WHERE TO_CHAR(dre.fechaenvio,'YYYYMM') = TO_CHAR(
      ...
      AND dre.IdDesembolsodetalle IN (1,6,7)
) a
WHERE a."CORTE DISPERSION" > (SELECT TO_CHAR(primer_dia_habil_mes,'YYMMDD') || '.0900' FROM fechas_habiles)
-- AND a.Solicitud <> 1219460   -- comentado en el original, sin efecto
ORDER BY 10 ASC;
