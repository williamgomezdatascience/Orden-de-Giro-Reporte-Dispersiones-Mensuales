/* ============================================================================
   ...
   - Es un query de agregacion (fila de totales). No requiere GROUP BY porque
     todas las columnas no literales usan SUM(); las columnas literales ''
     se preservan en su posicion exacta (31 columnas en total) porque
     GoAnywhere mapea el archivo de salida por posicion, no por nombre.
   - A diferencia de DETALLE_DESEMBOLSO y DETALLE_GIRO, este query 1_v2 NO
     filtra por dia de la semana (no tiene el filtro "datepart(dw,getdate())
     not in (1,7)"); se preserva tal cual.
   - Se preserva el uso de "getdate()-17" en vez de la fecha ancla del mes
     (quirk original, no se corrige la logica de negocio).
   - El join hacia Seguros es LEFT JOIN (en la variante 2_v2 es INNER JOIN);
     ver NOTAS_MIGRACION.md para el detalle de ambas variantes.
   - OJO: en Oracle, una columna seleccionada como '' (cadena vacia) se
     evalua como NULL, no como cadena vacia. Si GoAnywhere/el archivo plano
     de salida distingue entre NULL y "" (por ejemplo al exportar a CSV con
     comillas), validar el comportamiento; si se requiere una cadena vacia
     real, reemplazar '' por algo como CAST(' ' AS VARCHAR2(1)) o ajustar la
     plantilla de salida. No se modifica aqui porque escapa a la sintaxis SQL.
   ============================================================================ */
...
SELECT
    ...
    '',                                                                                                  -- 21 (duplicado comentado en original, no usado)
...
WHERE TO_CHAR(dre.fechaenvio,'YYYYMM') = TO_CHAR(
          CASE WHEN TRUNC(fh.hoy - 17) = fh.primer_dia_mes THEN ADD_MONTHS(fh.primer_dia_mes,-1) END,
      'YYYYMM')
